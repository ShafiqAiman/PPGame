...
dragonhealth :- findall(A, health1(1, A), L), head(L, B), C is B - 40, retractall(health1(_, _)), assertz(health1(1, C)).
...
wins(Ulst) :-	(Ulst = [_,_,_,_,kk,_,_,_,_] -> nl,write('You have hit the crucial part of the dragon.'),nl,
				write('The dragon health is deducted'),nl,dragonhealth,healthdeduction,dragon); nl,write('You did not hit the crucial part'),nl,write('You are hit by the dragon'),healthdeduction,nl,ques3.

dragon :- nl,write('The dragon attacked you back. Your health is deducted.'),findall(A, health1(1, A), L), head(L, B),(B > 0 -> ques3);
